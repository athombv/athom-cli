...
class App {
    ...
    const token = await AthomApi.createDelegationToken({
      audience: 'apps',
    });

    // TODO: Replace with AthomAppsAPI
		Log(colors.green(`✓ Submitting ${id}@${version}...`));
    ...
    if( Object.keys(env).length ) {
      ...
      Log(colors.grey(` — Homey.env (env.json)`));
  		Object.keys(env).forEach(key => {
    		const value = env[key];
        Log(colors.grey(`   — ${ellipsis(key)}=${ellipsis(value)}`));
  		});
		}

    const res = await fetch(`https://apps-api.office.athom.com/api/v1/app/${appJson.id}/build`, {
      method: 'post',
      headers: {
    ...
    if(!res.ok)
      throw new Error(json.error || res.statusText);
		...
		Log(colors.green(`✓ Uploading ${id}@${version} (${filesize(size)})...`));
    {
    ...
    // TODO git tag
		...
		Log(colors.green(`✓ App ${id}@${version} successfully uploaded. Visit the Developer Console to view the status of your app at https://developer.athom.com`));
  }
