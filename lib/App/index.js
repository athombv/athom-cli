...
class App {
	...
	async _switchToCompose(appPath) {
		...
		// Create all the required JSON objects
		const pluginJson = [
			{
				id: "compose"
			}
	...
	async createFlow() {
		...
		console.log('Answers before args', answers);
		...
		console.log(useArgs.arguments);

		let flowArgs;
		if (useArgs.arguments) {
		...
		console.log(useTokens.arguments);

		let tokenArgs;
		if (useTokens.arguments) {
			tokenArgs =  await inquirer.prompt([].concat(
				[
					{
						type: 'list',
						name: 'type',
					...
					},
					{
						type: 'input',
						name: 'placeholder',
						message: 'Enter the hint for the token',
						validate: input => {
							return input.length > 0;
						}
					}
