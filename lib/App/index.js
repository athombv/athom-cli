...
class App {
  ...
  async _getPrunePaths() {
    ...
    try {
      // Perform npm prune dry run and return set of paths to ignore during tar/upload
      const { stdout } = await exec(`npm prune --production --dry-run --json`, { cwd: this.path });
      prunePaths = JSON.parse(stdout);
      prunePaths = prunePaths.removed.map(pruneObject => path.resolve(pruneObject.path));
      return new Set(prunePaths);
    } catch(err) {

      // Check for npm not installed error
      if (err && Object.prototype.hasOwnProperty.call(err, 'message') &&
        err.message.includes('npm: command not found')) {
  ...
  async _getPackStream() {
    return tmp.file().then( async o => {
      ...
      const prunePaths = await this._getPrunePaths();
          ...
          if (name.startsWith(path.join(this.path, 'node_modules')) && prunePaths.has(name)) return true;
  ...
  static async create({ appPath }) {
    ...
    Log(colors.green(`✓ Installing npm packages...`));

    // Install homey as dep dependency
    await npm.install(Object.keys(packageJson.devDependencies), {
      cwd: appPath,
      save: true,
    });
