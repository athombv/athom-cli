...
class App {
	...
	async preprocess() {
		...
		const appJson = await this._folderHasAppJson();
		if (appJson) {
	 ...
	 */
  	async _folderHasAppJson(jsonPath) {
	...
	async _isComposeEnabled() {
		// Test if the Homey compose folder exists, otherwise switch to compose first or cancel the operation
		if( await fse.exists( path.join(this.path, '.homeycompose') ) ) {
			console.debug('.homeycompose folder found!');
			return true;
		} else {
			...
			if( answers.switch_compose ) {
				// TODO implement switchToCompose somewhere
				// await this._switchToCompose();
				return true;
			} else throw new Error('Command requires Homey compose which is not enabled!');
		}
	}
	...
	static async create({ appPath }) {
		...
		if( answers.compose ) {
			...
			dirs.push( path.join('.homeycompose', 'flows') );
