...
class App {
	...
	async switchToCompose() {
		...
		composeDirs.forEach(async dir => {
			try {
				await mkdirAsync( path.join(this.path, dir) );
			} catch( err ) {
				Log( err );
			}
		});
	...
	async addDriverCapability() {
		...
		// capabilitesAnswers.capabilities.forEach(newCapability => {
		// 	if( !driverJson.capabilities.some(capability => capability === newCapability))
		// 		driverJson.capabilities.push(newCapability);
		// });
