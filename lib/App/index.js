...
class App {
	...
	async preprocess() {		
		...
		try {
			appJson = path.join( this._path, 'app.json' );
			appJson = await readFileAsync( appJson, 'utf8' );
			appJson = JSON.parse( appJson );
		} catch( err ) {
			if( err.code === 'ENOENT' )
				throw new Error(`Could not find a valid Homey App at \`${this._path}\``);
				
			throw new Error(`Error in \`app.json\`:\n${err}`);
		}
				
		if( !Array.isArray(appJson.plugins) || appJson.plugins.length < 1 ) return;
		
		Log(colors.green('✓ Running plugins...'));
		
		for( let i = 0; i < appJson.plugins.length; i++ ) {
			let plugin = appJson.plugins[i];
