...
class App {
  ...
  async publish() {
    ...
    if ( await GitCommands.isGitRepo({path: this.path}) ) {
  ...
  async migrateToCompose() {
    if( this._hasPlugin('compose') ) return new Error('You already have the Compose plugin enabled, no need to run this command.');
    ...
    if( appJson.capabilities ) {
      ...
      try {
        if ( !await fse.exists( path.join(homeyComposePath, 'capabilities') ) ) await mkdirAsync( path.join(homeyComposePath, 'capabilities') );
      } catch( err ) { console.log('Error creating folder', err) }

      Object.entries(appJson.capabilities).forEach( async ([name, capability]) => {
        try {
          await writeFileAsync(
            path.join(homeyComposePath, 'capabilities', `${name.toLowerCase()}.json`),
