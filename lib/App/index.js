...
class App {
	...
	*/
	...
	async migratoToCompose() {
		...
		// Flow seperation
		if( appJson.flow ){
			const appFlowJson = appJson.flow;
			...
			[ 'triggers', 'conditions', 'actions' ].forEach( async type => {
				if( appFlowJson[type] ){
					try {
					...
					appFlowJson[type].forEach( async flowCard => {
						flowCard.args.forEach( argument => {
							if( argument.hasOwnProperty( 'filter' ) && argument.filter.includes('driver') ) {
								Log(`Flow Card '${flowCard.id}' is using a driver filter. You might want to create a separate '\driver.flow.compose.json'\ in the driver folder.`);
							}
						});
