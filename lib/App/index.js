...
class App {
	 ...
	 */
  	async _getAppJsonFromFolder(folderPath) {
	...
	async switchToCompose() {
		...
		const driverFolderContents = await readDirAsync(driverPath, { withFileTypes: true });
		let drivers = [];

		driverFolderContents.forEach( content => {
			if( content.isDirectory() ) { drivers.push(content.name) }
		});
		...
		let composeDriverJson, composeFlowsJson, composeDiscoveryJson;
		if( appJson.drivers){
			composeDriverJson = appJson.drivers;
			delete appJson.drivers;
		}
		if( appJson.flow) {
			composeFlowsJson = appJson.flow;
			delete appJson.flow;
			composeDirs.push( path.join('.homeycompose', 'flow') );
		}
		if( appJson.discovery ) {
			composeDiscoveryJson = appJson.discovery;		
			delete appJson.discovery;
			composeDirs.push( path.join('.homeycompose', 'discovery') );
		}

		const composeAppJson = appJson;

		// Start writing folders and files
		composeDirs.forEach(async dir => {
			try {
				if ( !await fse.exists( path.join(this.path, dir) ) ) await mkdirAsync( path.join(this.path, dir) );
			} catch( err ) { console.log('Error creating folder', dir, err) }
		});

		if( drivers && composeDriverJson) {
			drivers.forEach(driver => {
				composeDriverJson.forEach(async driverObject => {
					if( driverObject.id === driver) {
						await writeFileAsync(
							path.join( driverPath, driver, 'driver.compose.json'), 
							JSON.stringify( driverObject, false, 2)
						);
						Log(`Created driver compose file ${driver}`);
					}
				});
			});
		}
		...
		// Separate flows
		if( composeFlowsJson.triggers ) {
			const triggerFolder = path.join(homeyComposePath, 'flow', 'triggers');
			if ( !await fse.exists( triggerFolder ) ) await mkdirAsync( triggerFolder );

			composeFlowsJson.triggers.forEach( async trigger => {
				if( trigger.args) {
					trigger.args.forEach( argument => {
						if( argument.hasOwnProperty( 'filter' ) && argument.filter.includes('driver') ) {
							Log(`Flow trigger ${trigger.id} is using a driver filter. Please create a separate driver.flow.compose.json in the driver folder.`);
						}
					});
				}
				try {
					await writeFileAsync(
						path.join(triggerFolder, `${trigger.id}.json`),
						JSON.stringify(trigger, false, 2)
					);
					Log(`Created Flow trigger ${trigger.id}.json`);
				} catch( err ) { console.log('Error writing flow trigger JSON', err) }
			})
		}

		if( composeFlowsJson.conditions ) {
			const conditionFolder = path.join(homeyComposePath, 'flow', 'conditions');
			if ( !await fse.exists( conditionFolder ) ) await mkdirAsync( conditionFolder );

			composeFlowsJson.triggers.forEach( async condition => {
				if( condition.args) {
					condition.args.forEach( argument => {
						if( argument.hasOwnProperty( 'filter' ) && argument.filter.includes('driver') ) {
							Log(`Flow condtion ${condition.id} is using a driver filter. Please create a separate driver.flow.compose.json in the driver folder.`);
						}
					});
				}
				try {
					await writeFileAsync(
						path.join(conditionFolder, `${condition.id}.json`),
						JSON.stringify(condition, false, 2)
					);
					Log(`Created Flow condtion ${condition.id}.json`);
				} catch( err ) { console.log('Error writing flow condtion JSON', err) }
			})
		}

		if( composeFlowsJson.actions ) {
			const actionFolder = path.join(homeyComposePath, 'flow', 'actions');
			if ( !await fse.exists( actionFolder ) ) await mkdirAsync( actionFolder );

			composeFlowsJson.actions.forEach( async action => {
				if( action.args) {
					action.args.forEach( argument => {
						if( argument.hasOwnProperty( 'filter' ) && argument.filter.includes('driver') ) {
							Log(`Flow action ${action.id} is using a driver filter. Please create a separate driver.flow.compose.json in the driver folder.`);
						}
					});
				}
				try {
					await writeFileAsync(
						path.join(actionFolder, `${action.id}.json`),
						JSON.stringify(action, false, 2)
					);
					Log(`Created Flow action ${action.id}.json`);
				} catch( err ) { console.log('Error writing flow action JSON', err) }
			})
		}

		if( composeDiscoveryJson ) {
			Object.entries(composeDiscoveryJson).forEach( async ([name, strategy]) => {
				try {
					await writeFileAsync(
		...
		try {
			await writeFileAsync(
				...
				JSON.stringify(composeAppJson, false, 2)
			);
		} catch( err ) { console.log('Error writing files', err) }

		await this.addPlugin('compose');

		Log(colors.green(`✓ Succesfully switched app ${composeAppJson.id} to compose`));
	...
	async createDriver() {
		...
		if( answers.isIp === true && answers.createDiscovery ) {
	...
	async addDriverCapability() {
		...
		const driverFolder = path.join( this.path, 'drivers', selectedDriverAnswer.driverId );
		const driverJsonPath = path.join(driverFolder, 'driver.compose.json');
		...
		try {
			...
			if( err.code === 'ENOENT' )	throw new Error(`Could not find a valid driver.compose JSON at \`${driverJsonPath}\``);
		...
		Log(colors.green(`✓ Driver capabilities updated in \`${driverPath}\``));
	}

	async createDriverFlow() {
		// Path to list al driver from
		const driversFolder = path.join( this.path, 'drivers' );
		try {
			await fse.ensureDir( driversFolder );
		} catch( error ) {
			throw new Error('Your app doesn\'t contain any drivers!');
		}

		const folderContents = await readDirAsync(driversFolder, { withFileTypes: true });
		let drivers = [];
		...
		const flowPath = path.join(driversFolder, chosenDriver, 'driver.flow.compose.json');
		...
		if( !driverFlowJson[flowType] ) {
			driverFlowJson[flowType] = []; // Init empty array so that the chosen type is always pushable
		}
		...
		if (useArgs.using_arguments) {
			...
			async function addArgument() {
				...
				flowArgs.push({
					...
					placeholder: {
						en: argumentAnswers.placeholder
					}
				});
		...
		if (useTokens.using_tokens) {
			async function addToken() {
				...
				flowTokens.push({
					...
					title: {
						en: tokenAnswers.title
					},
					example : {
						en: tokenAnswers.example
					}
				});
	...
	async createFlow() {
		...
		// Property 'type' shouldn't end in the JSON so delete it.
		delete flowJson.type;
