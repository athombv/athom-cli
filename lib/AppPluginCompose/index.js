	...
	*/		
	...
	async _composeLocales() {
		...
		for( let appComposeLocaleId in appComposeLocales ) {
			...
			let appLocaleId = appComposeLocaleIdArray.shift();
			
			let localeObj = {};
			let latestRef = localeObj;
			for( let i = 0; i < appComposeLocaleIdArray.length; i++ ) {
				let appComposeLocaleIdItem = appComposeLocaleIdArray[i];
				
				if( i === appComposeLocaleIdArray.length - 1 ) {
					latestRef[appComposeLocaleIdItem] = appComposeLocale;
				} else {
					latestRef[appComposeLocaleIdItem] = latestRef[appComposeLocaleIdItem] || {};
					latestRef = latestRef[appComposeLocaleIdItem];
				}
				
			}
						
			appLocales[ appLocaleId ] = deepmerge( appLocales[ appLocaleId ], localeObj );
			appLocales[ appLocaleId ] = deepmerge( appComposeLocale, appLocales[ appLocaleId ] );
		}
		
		for( let appLocaleId in appLocales ) {
