...
class AthomApi {
	...
	async logout() {
		...
		return Settings.unset('athomToken');
	}
	...
	async getActiveHomey() {
		...
		this._activeHomey = this.getHomey( activeHomey.id ).then(homey => homey.authenticate() );
		return this._activeHomey;
	}
	
	async setActiveHomey({ id, name }) {
		return Settings.set('activeHomey', {
			id,
			name
		});
	}
