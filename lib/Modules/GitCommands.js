...
class GitCommands {
    ...
    async createTag ({version, message}) {
        try {
            ...
            const result = await this._executeGitCommand(`tag -a "${version}" -m"${message}`);
    ...
    async commitFile({file, message}) {
        return this._executeGitCommand(`commit ${file} -m ${message}`);
    }
    ...
    async push() {
        ...
        await this._executeGitCommand('git push');
    }
