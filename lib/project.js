...
function run_app( app_path, debug ) {
	app_path = ( typeof app_path == 'string' ) ? app_path : process.cwd();
	...
	function step2( homey ){
		...
		pack( app_path, function( tmppath ){
			upload( tmppath, homey, token, debug, env, function( err, response ){
				...
				if( debug ) {
					...
					var socket = socket_io_client( homey.localUrl + '/realtime/manager/devkit/', {
						query: 'token=' + token
					})
						.on('connect', function(){
							...
							if( session ) {
								...
								// get std buffer
								request({
									url: homey.localUrl + '/api/manager/devkit/std/' + session,
									headers: {
									...
									if( err ) throw( err );
									if( !(response && response.result ) ) throw new Error("invalid response");
								...
								// get the log buffer
								request({
									url: homey.localUrl + '/api/manager/devkit/log/',
									headers: {
							    		'Authorization': 'Bearer ' + token
									},
									json: true
					...
					process.stdin.on('keypress', function (ch, key) {
						if (key && key.ctrl && key.name == 'c') {
							stop( homey.localUrl, token, response.result.app_id, function(err, status, body){
	...
	function upload( tmppath, homey, token, debug, env, callback ) {

		var address = homey.localUrl;

		console.log( ++step + '/' + steps + ' -', 'Uploading to ' + homey.name.italic + ' @ ' + address.italic + '...');

		// POST the tmp file to Homey
		req = request.post({
			url: address + '/api/manager/devkit/',
			headers: {
	    		'Authorization': 'Bearer ' + token
			},
			json	: true
	...
	function stop( address, token, app_id, callback ) {
		...
		req = request.del({
			url: address + '/api/manager/devkit/' + app_id,
			headers: {
	    		'Authorization': 'Bearer ' + token
			},
			json: true
